...
from scipy.stats import gaussian_kde  # Imported for density plot
...
def main() -> None:
    ...
    for metric in metrics_to_plot:
        ...
        # ----------------------------
        # GLOBAL DENSITY PLOT: Loop over all Unique Models
        # ----------------------------
        if args.model == 'all':
            print(f"Collecting all values for {metric} across all unique models...")
            
            # 1. Identify all unique models in the original dataframe
            unique_models = df["Model"].unique()
            
            all_collected_values = []

            # 2. Loop through each model to collect values
            for model_name in unique_models:
                # Filter rows for this specific model and metric
                model_subset = df[(df["Model"] == model_name) & (df["Metric"] == metric)]
                
                # Iterate over every row found for this model
                for _, row in model_subset.iterrows():
                    val, _, _ = get_woman_stats(row)
                    all_collected_values.append(val)
            
            # 3. Plot the Density (KDE)
            if all_collected_values:
                nonzero_all = [v for v in all_collected_values if v != 0]
                # FILTER: Remove Zeroes AND Outliers (abs(val) > 5)
                clean_values = [v for v in nonzero_all if v != 0 and -5 <= v <= 5]
                nonzero_all = clean_values.copy()

                # We need enough data points and variance to plot a density
                if len(nonzero_all) > 1 and np.std(nonzero_all) > 1e-9:
                    plt.figure(figsize=(10, 6))
                    
                    # Calculate Kernel Density
                    kde = gaussian_kde(nonzero_all)
                    
                    # Create a grid of X values to plot against, with some buffer
                    x_min, x_max = min(nonzero_all), max(nonzero_all)
                    x_range = x_max - x_min
                    ...
                    plt.title(f'Density Plot of {metric} (All Unique Models, All Entries)')
                    plt.xlabel(metric)
                    plt.ylabel("Density")
                    plt.grid(axis='y', alpha=0.3)
                    plt.tight_layout()
                    
                    # Changed filename from histogram to density
                    plot_path = f"{out_root}/{metric}_global_density_all_unique_models.png"
                    print(f"Saving global density plot to {plot_path}")
                    plt.savefig(plot_path)
                    plt.close()
                else:
                    # Fallback to histogram if singular or not enough data
                    print("Not enough variance for density plot; falling back to histogram.")
                    plt.figure(figsize=(10, 6))
                    plt.hist(nonzero_all, bins=30, alpha=0.6, color="purple", edgecolor='black')
                    plt.title(f'Distribution of {metric} (All Unique Models, All Entries)')
                    plt.xlabel(metric)
                    plt.ylabel("Frequency")
                    plt.tight_layout()
                    plt.savefig(f"{out_root}/{metric}_global_histogram_all_unique_models.png")
                    plt.close()

        # ----------------------------
        # Grid Generation (Heatmap/Bar Chart logic)
        # ----------------------------
        ...
        # ----------------------------
        # Grid-Specific Histogram (Subset) - Left as Histogram
        # ----------------------------
        plt.figure(figsize=(10, 6))
