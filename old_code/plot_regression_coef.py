...
def calculate_difference(impact_ratio, threshold = 0.75, total_applicants=3200):
    ...
    women_selected = women_applicants * (1-threshold) * impact_ratio
...
os.makedirs(f'results/figs/{args.exp_framework}/{args.model}/', exist_ok=True)
...
#for metric in ["regression_coefficients"]:
    for condition_pair_key in condition_pairs.keys():
        ...
        r11 = get_row_or_default(df11, metric)
        r12 = get_row_or_default(df12, metric)
        r13 = get_row_or_default(df13, metric)
        r14 = get_row_or_default(df14, metric)
        r15 = get_row_or_default(df15, metric)
        r16 = get_row_or_default(df16, metric)
        r17 = get_row_or_default(df17, metric)
        r18 = get_row_or_default(df18, metric)
        r19 = get_row_or_default(df19, metric)
        r20 = get_row_or_default(df20, metric)
        r21 = get_row_or_default(df21, metric)
        r22 = get_row_or_default(df22, metric)
        r23 = get_row_or_default(df23, metric)
        r24 = get_row_or_default(df24, metric)
        r25 = get_row_or_default(df25, metric)
        r26 = get_row_or_default(df26, metric)
        r27 = get_row_or_default(df27, metric)
        r28 = get_row_or_default(df28, metric)
        r29 = get_row_or_default(df29, metric)
        r30 = get_row_or_default(df30, metric)
        r31 = get_row_or_default(df31, metric)
        r32 = get_row_or_default(df32, metric)
        r33 = get_row_or_default(df33, metric)
        r34 = get_row_or_default(df34, metric)
        r35 = get_row_or_default(df35, metric)
        r36 = get_row_or_default(df36, metric)
        r37 = get_row_or_default(df37, metric)
        r38 = get_row_or_default(df38, metric)
        r39 = get_row_or_default(df39, metric)
        r40 = get_row_or_default(df40, metric)
        r41 = get_row_or_default(df41, metric)
        r42 = get_row_or_default(df42, metric)
        r43 = get_row_or_default(df43, metric)
        r44 = get_row_or_default(df44, metric)
        r45 = get_row_or_default(df45, metric)
        r46 = get_row_or_default(df46, metric)
        r47 = get_row_or_default(df47, metric)
        r48 = get_row_or_default(df48, metric)
        r49 = get_row_or_default(df49, metric)
        r50 = get_row_or_default(df50, metric)
        r51 = get_row_or_default(df51, metric)
        r52 = get_row_or_default(df52, metric)
        r53 = get_row_or_default(df53, metric)
        r54 = get_row_or_default(df54, metric)
        r55 = get_row_or_default(df55, metric)
        r56 = get_row_or_default(df56, metric)
        r57 = get_row_or_default(df57, metric)
        r58 = get_row_or_default(df58, metric)
        r59 = get_row_or_default(df59, metric)
        r60 = get_row_or_default(df60, metric)
        r61 = get_row_or_default(df61, metric)
        r62 = get_row_or_default(df62, metric)
        r63 = get_row_or_default(df63, metric)
        r64 = get_row_or_default(df64, metric)
        r65 = get_row_or_default(df65, metric)
        r66 = get_row_or_default(df66, metric)
        r67 = get_row_or_default(df67, metric)
        r68 = get_row_or_default(df68, metric)
        r69 = get_row_or_default(df69, metric)
        r70 = get_row_or_default(df70, metric)
        ...
        # Only plot nonzero values in the histogram
        nonzero_values = [v for v in values if v != 0]
        plt.figure(figsize=(10, 6))
        plt.hist(nonzero_values, bins=30, alpha=0.5, color='blue')
        plt.title(f'Histogram of {metric} for "Woman" (nonzero values) ({args.sampling}, {args.analysis} {args.exp_framework} framework)')
        plt.xlabel(metric)
        plt.ylabel('Frequency')
        plt.tight_layout()
        plt.savefig(f'results/figs/{args.exp_framework}/{args.model}/{metric}_sampling_{args.sampling}_analysis_{args.analysis}_all_histogram_nonzero.png')
        plt.close()
        ...
        plt.savefig(f'results/figs/{args.exp_framework}/{args.model}/{metric}_sampling_{args.sampling}_analysis_{args.analysis}_all_heatmap.png')
        plt.close()
        ...
        plt.tight_layout()
        print(f'Saving figure to results/figs/{args.exp_framework}/{args.model}/{metric}_sampling_{args.sampling}_analysis_{args.analysis}_all.png')
        plt.savefig(f'results/figs/{args.exp_framework}/{args.model}/{metric}_sampling_{args.sampling}_analysis_{args.analysis}_all.png')
        plt.close()
...
heatmap_data.to_csv(f'results/figs/{args.exp_framework}/{args.model}/{metric}_sampling_{args.sampling}_analysis_{args.analysis}_all_heatmap_data.csv', index=False)
