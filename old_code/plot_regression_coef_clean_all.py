...
def main() -> None:
    ...
    # We will create a specific working_df for the grid logic, but keep the full df for the "all" histogram.
    ...
    for metric in metrics_to_plot:
        ...
        # ----------------------------
        # GLOBAL HISTOGRAM: Loop over all Unique Models
        # ----------------------------
        if args.model == 'all':
            ...
            # 3. Plot the aggregated list
            if all_collected_values:
                # Filter out zero values if desired (or keep them, based on preference. 
                # Usually removing strict zeros helps visualization if they are artifacts)
                nonzero_all = [v for v in all_collected_values if v != 0]

                plt.figure(figsize=(10, 6))
                plt.hist(nonzero_all, bins=50, alpha=0.6, color="purple", edgecolor='black')
                ...
                hist_path = f"{out_root}/{metric}_global_histogram_all_unique_models.png"
                print(f"Saving global histogram to {hist_path}")
